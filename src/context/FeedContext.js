...
const KEY = 'itemsDoFeed';


const rssItems =  [
    {
        title: 'Congresso e Planalto não chegam a acordo durante reunião para discutir Orçamento de 2021',
    ...
    {
        title: 'Ocupação de UTIs para adultos com Covid-19 em hospitais públicos de Fortaleza é a maior desde o início da pandemia',
    ...
    {
        title: 'Maranhão flexibiliza horário de funcionamento de bancos, lotéricas e supermercados',
...
const feedReducer = (state, action) => {
    ...
    switch (action.type) {
        ...
        case 'delete_item':
            newState = state.filter( (item) => item.link !== action.payload );
            deleteItemFeedStorage(newState);
            return newState;
...
const addItem = dispatch => {
    return (title, dataPublicacao, link, descricao, imagem, callback) => {
        dispatch({ type: 'add_item', payload: { title, dataPublicacao, link, descricao, imagem } });
        if (callback) {
            callback();
        }
    };
...
const restoreState = dispatch => async () => {
    try {
        ...
        if (!savedState) {
            saveStateItems(rssItems)
            console.log('Storage vazio [FeedContext] ');
        }
        else {
...
const saveStateItems = async (value) => {
    try {
...
const deleteItemFeedStorage = async () => {
    try {
